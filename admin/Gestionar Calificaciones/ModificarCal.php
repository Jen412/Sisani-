    ...
    <section>
        ...
        <form method="POST"><!-- Se debe de poner el metodo post como si fuese un formulario-->
            ...
            <div class = "container-table">
                    ...
                    <?php  
                        if (isset($_POST['btnRC'])) {//$_SERVER es una variable para determinar si en el formulario se ha enviado algo
                            ...
                            while($btnRC = mysqli_fetch_assoc($resultadoBtn)) : ?>
                                ...
                                <div class="table__item"><?php echo "<input type=\"number\" name \"modificar[]\">";?></div>
                    <?php endwhile; }?>
                    <input type="submit" value="Modificar Calificaciones" class="btnMC">
                    ...
                    <?php 
                        if(isset($_POST['btnMC'])){
                            ...
                            for ($i = 0; $i<sizeof($modificar); $i++){
                                
                                $ejecutarQuery("UPDATE calificaciones set calif $modificar[$i] FROM dficha as d
                                INNER JOIN calificaciones as cl
                                ON cl.alufic = d.alufic
                                INNER JOIN grupos as g
                                ON d.alufic = g.alufic
                                WHERE g.letraGrupo = '$grupo'AND cl.id_MateriaG
                                    ...
                                    WHERE mg.idMateria = $materia AND d.alufic
                                            ...
                                            ON c.idCar = d.carcve1 
                                            WHERE d.carcve1 = $carrera));");
                                $resultadoBtnM =mysqli_query($db, $ejecutarQuery);
